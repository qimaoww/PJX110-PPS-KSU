# OnePlus Ace 3 Pro PPS Profiles — v1.0.1
...
切换只通过 WebUI 完成。模块内置 A/B 两种 DTBO 内容族各自的原厂、33W、
55W 镜像；未知 DTBO SHA256 会拒绝写入。卸载模块时，如果任一物理槽仍
处于 33W/55W 配置，会按该槽当前的 DTBO 内容族恢复对应原厂镜像。

物理活动槽只用于确定写入 `dtbo_a` 还是 `dtbo_b` 分区。实际目标镜像由当前
DTBO 的完整 SHA256 所属内容族决定，不假定物理 A/B 槽必然对应 A/B 镜像族。
因此，同一固件因 OTA 或刷机历史导致两个槽的 DTBO 内容族互换时，模块仍会
沿用当前内容族切换和恢复；未知内容族仍会拒绝写入。
